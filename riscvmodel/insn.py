...
class InstructionBLTU(InstructionBType):
    def execute(self, model: State):
        if model.intreg[self.rs1].unsigned() < model.intreg[
                self.rs2].unsigned():
            model.pc = model.pc + self.imm
...
class InstructionLBU(InstructionILType):
    def execute(self, model: State):
        model.intreg[self.rd] = model.memory.lb(
            (model.intreg[self.rs1] + self.imm).unsigned())


@isa("lhu", RV32I, opcode=0b0000011, funct3=0b101)
class InstructionLHU(InstructionILType):
    def execute(self, model: State):
        model.intreg[self.rd] = model.memory.lh(
            (model.intreg[self.rs1] + self.imm).unsigned())
...
class InstructionSB(InstructionSType):
    def execute(self, model: State):
        model.memory.sb((model.intreg[self.rs1] + self.imm).unsigned(),
                        model.intreg[self.rs2])


@isa("sh", RV32I, opcode=0b0100011, funct3=0b001)
class InstructionSH(InstructionSType):
    def execute(self, model: State):
        model.memory.sh((model.intreg[self.rs1] + self.imm).unsigned(),
                        model.intreg[self.rs2])
...
class InstructionSW(InstructionSType):
    def execute(self, model: State):
        model.memory.sw((model.intreg[self.rs1] + self.imm).unsigned(),
                        model.intreg[self.rs2])
...
class InstructionSRLI(InstructionISType):
    def execute(self, model: State):
        model.intreg[self.rd] = model.intreg[self.rs1].unsigned() >> int(
            self.shamt)
...
class InstructionSLL(InstructionRType):
    def execute(self, model: State):
        model.intreg[self.rd] = model.intreg[self.rs1] << (
            model.intreg[self.rs2] & 0x1f)
...
class InstructionSRL(InstructionRType):
    def execute(self, model: State):
        model.intreg[
            self.rd] = model.intreg[self.rs1] >> model.intreg[self.rs2]
